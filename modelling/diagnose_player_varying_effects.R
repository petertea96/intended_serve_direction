...
# -- Last Edited: September 21st, 2021
...
# -- Model 4: Player-varying intercept and varying PI + common covariates
library(cmdstanr)
library(data.table)
options(mc.cores = 4)

{
atp_model1 <- cmdstan_model('./modelling/stan_files/common_intercept.stan')
atp_model2 <- cmdstan_model('./modelling/stan_files/player_varying_intercept.stan')
atp_model3 <- cmdstan_model('./modelling/stan_files/expanded_model.stan')
atp_model4 <- cmdstan_model('./modelling/stan_files/player_varying_intercept_and_covariate.stan')
}
# --- || --- || --- || --- || --- || --- || --- || 
...
atp.stanfit3 <- rstan::read_stan_csv(fit_atp_model3$output_files())
saveRDS(atp.stanfit3, file = "./modelling/saved_models/advi/advi_atp_model3.RDS")
...
# --- || --- || --- || --- || --- || --- || --- || 
atp.stanfit1 <- readRDS("./modelling/saved_models/advi/advi_atp_model1.RDS")
atp.stanfit2<- readRDS("./modelling/saved_models/advi/advi_atp_model2.RDS")
atp.stanfit3 <- readRDS("./modelling/saved_models/advi/advi_atp_model3.RDS")
atp.stanfit4 <- readRDS("./modelling/saved_models/advi/advi_atp_model4.RDS")

log_lik_1 <- extract_log_lik(atp.stanfit1)
log_lik_2 <- extract_log_lik(atp.stanfit2)
log_lik_3 <- extract_log_lik(atp.stanfit3)
log_lik_4 <- extract_log_lik(atp.stanfit4)
...
print(loo_compare(waic1, waic2, waic3, waic4))

# --- Plot player-varying covariate results
extract_stan_model <- extract(atp.stanfit4 )
