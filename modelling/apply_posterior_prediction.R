### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
### == Plot Expanded Model Predictive Distribution       =====
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
# -- Using our expanded model, obtain predictive distributions.
...
library(ggplot2)
library(ggridges)
source(file = "/Users/petertea/Documents/intended_serve_direction/src/ggplot_theme.R")
...
# -- Federer's Average serve lateral and longitudinal serve impact location
...
# --- || --- || --- || --- || --- || --- || --- || --- ||
# -- Prediction example on Roger Federer -----
# --- || --- || --- || --- || --- || --- || --- || --- ||
# Situation:
...
set.seed(824)
expanded_model_fit <- readRDS("./modelling/saved_models/stan_loglik/atp_expanded_fit.RDS")
extract_expanded_model <- extract(expanded_model_fit)

# -- Need `get_column_prediction()` function
...
# -- T Logit
prob_T_logit_1st <-
(extract_expanded_model$v_id1[,fed_id,1] + extract_expanded_model$B_0[,1]) + # Intercept
extract_expanded_model$B_1[,1]*0 + # 1st Serve
extract_expanded_model$B_2[,1]*fed_avg_lat_dist + # Average Lateral Distance on 1st serve
extract_expanded_model$B_3[,1]*fed_avg_long_dist + 
extract_expanded_model$B_4[,1]*1 + # Previous location is T
extract_expanded_model$B_5[,1]*0 + 
extract_expanded_model$B_6[,1]*0 + 
extract_expanded_model$B_7[,1]*0.1443323 + 
extract_expanded_model$B_8[,1]*0 + 
extract_expanded_model$B_9[,1]*1 + 
extract_expanded_model$B_10[,1]*0 + 
extract_expanded_model$B_11[,1]*0 

# -- Wide Logit
prob_Wide_logit_1st <-
(extract_expanded_model$v_id1[,fed_id,2] + extract_expanded_model$B_0[,2]) +
extract_expanded_model$B_1[,2]*0 + # 1st Serve
extract_expanded_model$B_2[,2]*fed_avg_lat_dist + # Average Lateral Distance on 1st serve
extract_expanded_model$B_3[,2]*fed_avg_long_dist + 
extract_expanded_model$B_4[,2]*1 + # Previous location is T
extract_expanded_model$B_5[,2]*0 + 
extract_expanded_model$B_6[,2]*0 + 
extract_expanded_model$B_7[,2]*0.1443323 + 
extract_expanded_model$B_8[,2]*0 + 
extract_expanded_model$B_9[,2]*1 + 
extract_expanded_model$B_10[,2]*0 + 
extract_expanded_model$B_11[,2]*0 
...
normalized_probs_1st <- apply(prob_logit_1st, MARGIN = 1, FUN = softmax) %>% t() 
...
prob_T_logit_2nd <-
  (extract_expanded_model$v_id1[,fed_id,1] + extract_expanded_model$B_0[,1]) + # Intercept
  extract_expanded_model$B_1[,1]*1 + # 1st Serve
  extract_expanded_model$B_2[,1]*fed_avg_lat_dist + # Average Lateral Distance on 1st serve
  extract_expanded_model$B_3[,1]*fed_avg_long_dist + 
  extract_expanded_model$B_4[,1]*1 + # Previous location is T
  extract_expanded_model$B_5[,1]*0 + 
...
prob_Wide_logit_2nd <-
  (extract_expanded_model$v_id1[,fed_id,2] + extract_expanded_model$B_0[,2]) +
  extract_expanded_model$B_1[,2]*1 + # 1st Serve
  extract_expanded_model$B_2[,2]*fed_avg_lat_dist + # Average Lateral Distance on 1st serve
  extract_expanded_model$B_3[,2]*fed_avg_long_dist + 
  extract_expanded_model$B_4[,2]*1 + # Previous location is T
  extract_expanded_model$B_5[,2]*0 + 
