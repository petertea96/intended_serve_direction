...
### ==    PLOT MODEL RESULTS          =====
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
# -- After fitting all bayesian models, let's plot the results
# -- This script is also used to obtain posterior means and SE for 
# -- result tables.
# Last Edited: Sept 22, 2021
...
extract_stan_model <- extract(expanded_model_fit)
names(extract_stan_model)

player_varying_intercepts <- extract_stan_model$v_id1
#player_varying_intercepts <- extract_stan_model$v_intercept
...
#atp_sd_params <- extract_stan_model$sigma_intercept
...
#atp_rho_param <- extract_stan_model$Rho_intercept
...
# -- ATP COVARIATE EFFECTS (Expanded Model) -----
# -- Serve Number
...
wta_expanded_model <- readRDS("./modelling/saved_models/stan_loglik/wta_expanded_fit.RDS")
wta_extract_stan_model <- extract(wta_expanded_model)


# -- WTA COVARIATE EFFECTS (Expanded Model) -----
# -- Serve Number
...
#wta_player_varying_intercepts <- wta_extract_stan_model$v_intercept
...
#wta_sd_params <- wta_extract_stan_model$sigma_intercept
...
#wta_rho_param <- wta_extract_stan_model$Rho_intercept
